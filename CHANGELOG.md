...
### Fixed
- **Dynamic UI Positioning:** The sort button now dynamically updates its position every frame. This fixes the issue where the button would stay in its old position when the Recipe Book was opened in the Crafting Table or Player Inventory.

## [1.1.7] - 2026-06-12

### Fixed
- **Definitive Icon Visibility:** Generated high-resolution (512px) and Mod Menu (64px) icons with solid backgrounds to ensure visibility on GitHub and in-game.
- **GitHub Link:** Corrected the repository sources link in `fabric.mod.json`.

## [1.1.6] - 2026-06-12

### Added
- **Dual Sorting:** When opening a container (like a Chest), clicking the sort button or pressing 'R' now simultaneously sorts BOTH the container's inventory AND the player's main inventory as two separate actions.

## [1.1.5] - 2026-06-12

### Fixed
- **Crafting Table UI Fix:** Fixed a bug where clicking the sort button inside non-chest screens (like the Crafting Table or Furnace) would attempt to sort the crafting grid/inputs instead of the player's main inventory.

## [1.1.4] - 2026-06-12

### Fixed
- **UI Overlap:** Removed the text "S" overlay from the sort button that was overlapping the "Cool S" icon.

## [1.1.3] - 2026-06-12

### Added
- **Item Stack Merging:** Items are now automatically merged into full stacks (matching ID and components) before being sorted alphabetically.
